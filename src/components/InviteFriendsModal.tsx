...
import { buildInviteRejectionDocId, buildRideInviteDocId, type RideInviteKind } from '../lib/rideInvites';
...
export default function InviteFriendsModal({
  ...
  const invite = async (friendUid: string) => {
    ...
    try {
      ...
      if (!groupSnap.exists()) {
        showMessage({
          ...
          title: 'Código de ruta',
          message:
            'No hay ningún grupo con ese código en la base de datos. Revisa el código en pantalla.',
        });
        return;
      }
      ...
      const memberArr: string[] = Array.isArray(rawMembers)
        ? rawMembers.map((x: unknown) => String(x).trim()).filter(Boolean)
        : [];
    ...
    try {
      ...
      const inviteDocId = buildRideInviteDocId(user.uid, canonUid, routeCode);
      const rejectionId = buildInviteRejectionDocId(user.uid, routeCode);
      const rejectionSnap = await getDoc(doc(db, 'users', canonUid, 'inviteRejections', rejectionId));
      ...
      const ref = doc(db, 'rideInvites', inviteDocId);
      await setDoc(ref, {
        fromUid: user.uid,
        toUid: canonUid,
        groupId: routeCode,
      ...
      if (err?.code === 'permission-denied') {
        showMessage({
          ...
          title: 'Invitación',
          message:
            'No se pudo enviar. Comprueba que sigáis siendo amigos o tengáis chat directo, y que seas miembro de esta ruta en el servidor.',
        });
        return;
      }
      handleFirestoreError(e, OperationType.WRITE, `rideInvites/${targetUid}`);
    } finally {
      setSendingId(null);
    }
  };
            ...
            <div className="px-5 py-3 overflow-y-auto flex-1 custom-scrollbar">
              <p className="text-xs text-zinc-400 mb-3 leading-relaxed">
                {inviteKind === 'scheduled_ride' ? (
                  ...
                  <>
                    La invitación se guarda en su bandeja (icono arriba a la derecha en el inicio) y pueden unirse sin
                    salir de MotoRide.
                  </>
                )}
              </p>
            ...
            {inviteKind === 'scheduled_ride' && (
              <div className="shrink-0 space-y-3 border-t border-zinc-800 bg-zinc-950/70 px-5 py-4">
                <p className="text-[11px] leading-relaxed text-zinc-500">
                  ...
                  <span className="text-zinc-300">enlace</span> para que otros se apunten desde MotoRide (mismo formato
                  que al crear la ruta).
                </p>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const { clipboardText } = buildScheduledInviteSharePayload({
                        routeName: (groupName || 'Ruta').trim() || 'Ruta',
                      ...
                      if (ok) {
                        ...
                        window.prompt('Copia el texto:', clipboardText);
                      }
                    }}
                    className="flex items-center justify-center gap-2 rounded-xl bg-orange-500/90 py-2.5 text-xs font-bold text-white hover:bg-orange-500"
                  >
